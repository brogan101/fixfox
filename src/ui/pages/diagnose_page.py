...
class DiagnosePage(PageScroll):
    ...
    def _build_ui(self) -> None:
        ...
        w.diag_issue_summary = Card("Issue-family Triage", "Issue-aware triage bundles, diagnostics, and next-best actions.", elevation=2)
        ...
        w.diag_issue_scope = QComboBox()
        w.diag_issue_scope.addItem("All Coverage", "all")
        w.diag_issue_scope.addItem("Deep Playbooks", "deep")
        w.diag_issue_scope.addItem("Admin Required", "admin")
        w.diag_issue_scope.addItem("Network Needed", "network")
        w.diag_issue_scope.addItem("Restart Risk", "restart")
        w.diag_issue_family.currentTextChanged.connect(w._refresh_support_diagnose)
        w.diag_issue_scope.currentTextChanged.connect(w._refresh_support_diagnose)
        ...
        w.diag_issue_summary.body_layout().addWidget(w.diag_issue_scope)
        left_layout.addWidget(w.diag_issue_summary)
        w.diag_summary = Card("No active session", "Run Quick Check from Home.", elevation=2)
        ...
        w.diag_next = Card("Next Best Action", "Run Quick Check to generate findings.", right_widget=w.diag_next_btn, elevation=2)
        ...
        w.diag_playbook_summary = Card("Deep Playbook Run", "Issue-aware diagnostics, latest findings, and next actions appear here.", elevation=2)
        ...
        toolbar = Card("Findings Toolbar", "Search and filter findings.", elevation=2)
        toolbar_row = QWidget()
